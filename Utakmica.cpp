...
void Utakmica::operator++()
{
    ...
    if (x == 0) {
        cout << "-----------------------------------------\n";
        cout << this->getDomaci().getNaziv() << " " << Utakmica::brKosevaDomaci << " : " << Utakmica::brKosevaGosti << " " << this->getGosti().getNaziv() << "\n";
...
{
    stream <<  "\n-----------------------------------------\n";
    stream << u.getDomaci().getNaziv() << " " << Utakmica::brKosevaDomaci << " : " << Utakmica::brKosevaGosti << " " << u.getGosti().getNaziv() << "\n";
    stream << "-----------------------------------------\n";
    for (int i = 0; i < u.getDomaci().getIgraci().size(); i++) {
        stream << u.getDomaci().getIgraci()[i] << "\t\t" << u.getGosti().getIgraci()[i] << "\n";
    }
    return stream;
}
